...
#[cfg(feature = "no_as")]
const IMA_BASE_DIR: &str =
    "/etc/attestation/attestation-agent/local_verifier/virtcca/ima";
...
impl VirtCCAImaVerify {
    fn get_ima_reference_file_path(app_id: Option<&str>) -> String {
        if let Some(app_id) = app_id {
            if !Self::is_valid_rim_hex(app_id) {
                log::warn!("invalid rim app_id {}, fallback to default reference", app_id);
    ...
    fn is_valid_rim_hex(s: &str) -> bool {
        matches!(s.len(), RIM_HEX_LEN_SHA256 | RIM_HEX_LEN_SHA384 | RIM_HEX_LEN_SHA512)
