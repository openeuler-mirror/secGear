...
impl PolicyEngine for OPA {
    ...
    fn evaluate(
        ...
        for id in policy_id_used {
            ...
            path.push(id.clone());
            let engine_policy = std::fs::read_to_string(path.clone()).map_err(|err| {
    ...
    fn set_policy(&self, policy_id: &String, policy: &String) -> Result<(), PolicyEngineError> {
        ...
        std::fs::write(policy_file.as_path(), &raw).map_err(|err| {
    ...
    fn get_all_policy(&self) -> Result<HashMap<String, String>, PolicyEngineError> {
        let mut items = std::fs::read_dir(&self.policy_dir.as_path()).map_err(|err| {
            PolicyEngineError::ReadPolicyError(format!("read policy failed: {}", err))
        })?;
        ...
        while let Some(item) = items.next() {
            ...
            if path.extension().and_then(std::ffi::OsStr::to_str) == Some("rego") {
                let content: String = std::fs::read_to_string(path.clone()).map_err(|err| {
                    PolicyEngineError::ReadPolicyError(format!("read policy failed: {}", err))
                })?;
                let name = path
                    .file_stem()
    ...
    fn get_policy(&self, policy_id: &String) -> Result<String, PolicyEngineError> {
        let mut policy_file: PathBuf = self.policy_dir.clone();
        policy_file.push(format!("{}", policy_id));
        let policy = std::fs::read(policy_file.as_path()).map_err(|err| {
            PolicyEngineError::ReadPolicyError(format!("read policy failed: {}", err))
        })?;
