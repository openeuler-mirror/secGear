...
HOST_BIN="${BUILD_DIR}/secgear_starterkit_host"
...
if [[ ! -e /dev/sgx_enclave && ! -e /dev/sgx_provision && ! -e /dev/isgx ]]; then
  ...
  echo "Build is successful, but runtime validation requires SGX-enabled hardware."
  exit 2
fi

echo "[3/3] run host program"
cd "${BUILD_DIR}"
LD_LIBRARY_PATH=/usr/lib64:/opt/intel/sgxsdk/lib64 ./secgear_starterkit_host
