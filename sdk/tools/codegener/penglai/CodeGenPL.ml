...
let create_args_header (ec : enclave_content) (flags : int) =
    ...
    output_string output (
            String.concat "\n" (GenheaderPL.generate_args_header ec));
    close_out output
...
let create_trusted_header (ec : enclave_content) =
    ...
    let output = open_out file_name in
    output_string output (
            String.concat "\n" (GenheaderPL.generate_trusted_header ec));
    close_out output
...
let create_untrusted_header (ec : enclave_content) =
    ...
    let output = open_out file_name in
    output_string output (
            String.concat "\n" (GenheaderPL.generate_untrusted_header ec));
    close_out output
...
let check_trust_funcs_method (tfs : trusted_func list) (ep : edger8r_params)=
    if ep.use_prefix then failwithf "PengLai mode is not support --use_perfix option";
    List.iter
        (fun t ->
            if t.tf_is_priv then
                failwithf "%s :PengLai mode is not support 'private' feature"
                    t.tf_fdecl.fname;
            if t.tf_is_switchless then
                failwithf "%s :PengLai mode is not support 'switchless' feature"
                    t.tf_fdecl.fname;
            if check_is_user_check t.tf_fdecl then
                failwithf "%s :PengLai mode is not support 'user_check' feature"
                    t.tf_fdecl.fname)
            tfs
    ...
    List.iter
        (fun t ->
            if t.uf_fattr.fa_dllimport then
                failwithf "%s:PengLai mode is not support dllimport\n"
                t.uf_fdecl.fname;
            if t.uf_allow_list != [] then
                printf "WARNING: %s: Reentrant ocalls are not supported by Penglai. Allow list ignored.\n"
                t.uf_fdecl.fname;
            ...
            if t.uf_fattr.fa_convention <> CC_NONE then
                ...
                printf "WARNING: %s: PengLai mode is not support Calling convention %s for ocalls\n"
...
let generate_penglai_enclave_code (ec : enclave_content) (ep : edger8r_params) =
