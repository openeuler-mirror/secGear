...
let set_ocall_func (uf : untrusted_func) =
    ...
    let params_point = CommonfuncPL.set_parameters_point ufd in
    let out_params = CommonfuncPL.set_out_params ufd in
    [
        ...
        "";
        "    " ^ concat "\n    " (CommonfuncPL.set_in_params ufd);
        "";
        "    /* Fill return val, out and in-out parameters */";
        if out_params <> ["";""] then
        "    " ^ concat "\n    " (CommonfuncPL.set_out_params ufd)
        ...
        "";
        "    /* Call host function */";
        "    " ^ concat "    \n" (CommonfuncPL.set_call_user_func ufd);
